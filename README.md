...
新原作角色会在正文生成前完成检索（可配置最长等待时间，超时自动转入后台、下一轮补全）：候选译名只作为线索，搜索结果可以校正正式姓名和所属作品；核实后的资料会先写入本卡资料库，再由分析模型压缩成“按当前时间线过滤”的紧凑档案（节点之后的剧情、关系变化与秘密会被剔除）注入提示。已有角色直接复用缓存，只有明确 AU 差异或重大时间线变化才做增量更新；档案随资料或时间线变化自动重新压缩，分析模型不可用时自动回退为截断的原始摘录。
...
正文生成完毕后，插件会把刚生成的消息与本卡资料档案交给分析模型审核：凡是与档案冲突、且角色卡、用户指示、已确认AU差异和此前剧情都没有解释过的内容（姓名译名、年龄身份、外貌、性格行为、能力、经历、人际关系、时间线之后的剧情等），会自动按档案最小化修订并更新消息；此前剧情已明确建立的事实视为有效设定，不会被改动。审核只处理本轮登场角色相关消息，修订记录会显示在插件报告里。
